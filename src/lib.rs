...
impl<F: PrimeField, P: DenseUVPolynomial<F>, H: Hash<F>> FRI_LDT<F, P, H> {
    ...
    pub fn prove(p: &P) -> LDTProof<F> {
        ...
        let constant_fL_l: F = fL_i.coeffs()[0];
        let constant_fR_l: F = fR_i.coeffs()[0];
    ...
    pub fn verify(
        ...
        let mut i_z = 0;
        for (mut i_z, i) in (0..proof.evals.len()).step_by(2).enumerate() {
            let alpha_i = transcript.get_challenge(b"get alpha_i");
            ...
            if i < proof.evals.len() - 2 && next_fi_z2 != proof.evals[i + 2] {
                println!(
